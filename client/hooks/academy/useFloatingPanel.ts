...
export function useFloatingPanel({ panelId }: UseFloatingPanelProps) {
  ...
  // Refs for resize handlers (React 19 forward reference pattern)
  const handleResizeMoveRef = useRef<(event: MouseEvent) => void>(() => {});
  const handleResizeTouchMoveRef = useRef<(event: TouchEvent) => void>(() => {});
  ...
  const startResize = useCallback(
    (
      ...
      // Add global event listeners (using refs for React 19 forward reference compliance)
      ...
      document.addEventListener('touchmove', handleResizeTouchMoveRef.current);
  ...
  const handleResizeEnd = useCallback(() => {
    ...
    document.removeEventListener('touchmove', handleResizeTouchMoveRef.current);
    document.removeEventListener('touchend', handleResizeEndRef.current);
  }, []);

  // Sync resize handlers to refs (React 19 forward reference pattern)
  useLayoutEffect(() => {
    handleResizeMoveRef.current = handleResizeMove;
  }, [handleResizeMove]);

  useLayoutEffect(() => {
    handleResizeTouchMoveRef.current = handleResizeTouchMove;
  ...
  // Clean up event listeners on unmount (using refs for stable references)
  useEffect(() => {
    // Capture current ref values for cleanup
    const resizeMove = handleResizeMoveRef.current;
    const resizeEnd = handleResizeEndRef.current;
    const resizeTouchMove = handleResizeTouchMoveRef.current;
