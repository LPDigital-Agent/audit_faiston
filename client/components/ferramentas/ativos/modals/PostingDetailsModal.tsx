            ...
            <Dialog.Content asChild>
              ...
              >
                ...
                <div className="space-y-5">
                  {/* Ticket/Order Code & Status */}
                  <div className="flex items-center justify-between p-4 rounded-xl bg-white/[0.02] border border-white/[0.04]">
                    <div>
                      <p className="text-lg font-semibold text-white">
                        {order.tifluxTicketNumber ? `#${order.tifluxTicketNumber}` : order.codigo}
                      </p>
                      {order.tifluxTicketTitle && (
                        <p className="text-sm text-gray-400 mt-0.5">{order.tifluxTicketTitle}</p>
                      )}
                      <p className="text-xs text-gray-500 mt-1">{order.codigo}</p>
                    </div>
                    {statusConfig && (
