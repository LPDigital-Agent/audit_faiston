    ...
    <div className="space-y-6">
      ...
      <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-6 space-y-4">
        <p className="text-white/70">
          {message || 'Os seguintes campos não existem no banco de dados:'}
        </p>
      ...
      <GlassCard>
        ...
        <GlassCardContent>
          <div className="space-y-4">
            ...
            {/* BUG-039: Show DebugAnalysisPanel if debug analysis available */}
            {state.debugAnalysis ? (
              <DebugAnalysisPanel
                analysis={state.debugAnalysis}
      ...
      <GlassCard>
        <GlassCardHeader>
          <div className="flex items-center justify-between">
            ...
            {/* BUG-022 FIX: Split confidence display - File Quality vs Schema Mapping */}
            <div className="flex items-center gap-2">
              {/* File Quality (Phase 2) */}
              <Badge
                className={
                ...
                title="Qualidade do arquivo (estrutura, formato, legibilidade)"
              >
                📄 {Math.round(state.analysis.overallConfidence * 100)}%
              </Badge>
              {/* Schema Mapping (Phase 3) */}
              <Badge
                className={
                  state.analysis.mappingComplete
                    ? state.analysis.mappingConfidence !== undefined && state.analysis.mappingConfidence >= 0.8
                      ? 'bg-blue-500/20 text-blue-400'
                      : state.analysis.mappingConfidence !== undefined && state.analysis.mappingConfidence >= 0.5
                      ? 'bg-yellow-500/20 text-yellow-400'
                      : 'bg-orange-500/20 text-orange-400'
                    : 'bg-purple-500/20 text-purple-400 animate-pulse'
                }
                title={state.analysis.mappingComplete
                  ? 'Confiança no mapeamento de colunas para o schema do banco'
                  : 'Aguardando mapeamento de colunas...'}
              >
                {state.analysis.mappingComplete
                  ? `🔗 ${Math.round((state.analysis.mappingConfidence ?? 0) * 100)}%`
                  : '🔗 Pendente...'}
              </Badge>
            </div>
          </div>
