...
def _transform_file_structure_to_nexo_response(
    file_structure: dict,
    s3_key: str
) -> dict:
    """
    ...
    BUG-022 FIX: Standardizes Mode 2.5 response to match frontend contract.
    Previously, Mode 2.5 returned flat structure (columns, sample_data) while LLM
    path returned nested structure (analysis.sheets), forcing frontend to implement
    141-line normalization workaround.

    This function applies the "Sandwich Pattern" (CODE → LLM → CODE):
    - Tool returns deterministic flat structure
    - This function transforms to nested structure
    - Frontend receives consistent contract
    ...
    response = NexoAnalyzeFileResponse(
        ...
        overall_confidence=0.0,  # Zero - calculated by SchemaMapper in Phase 3
...
def _handle_nexo_analyze_file(payload: dict) -> dict:
    ...
    # BUG-022 FIX: Transform flat structure to nested before returning
    # This ensures frontend receives consistent nested structure (analysis.sheets)
    # regardless of code path (Mode 2.5 direct or LLM analysis)
    transformed = _transform_file_structure_to_nexo_response(result, s3_key)

    # Return in standard envelope format with transformed nested structure
    return {
        "success": True,
        "specialist_agent": "analyst",
        "response": transformed,  # ← Now returns nested structure matching TypeScript contract
    }
