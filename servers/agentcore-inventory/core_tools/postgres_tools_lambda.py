...
def debug_error(exception: Exception, operation: str, context: dict = None) -> dict:
    ...
    Returns:
        Empty dict (no enrichment available in Lambda context)
    """
    logger.error(f"[{operation}] {type(exception).__name__}: {exception}")
    if context:
        logger.error(f"[{operation}] Context: {context}")
    # Return empty dict to maintain API compatibility with callers
    # that expect enrichment data (e.g., lines 429, 473, 509, 600)
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if actual_tool not in handlers:
            unknown_tool_error = ValueError(f"Unknown tool: {actual_tool}")
            debug_error(unknown_tool_error, "postgres_tools_handler", {"tool_name": actual_tool, "available_tools": list(handlers.keys())})
            return {
                "content": [
                    {
                        "type": "text",
        ...
        debug_error(e, "postgres_tools_execute_tool", {"tool_name": actual_tool if 'actual_tool' in dir() else None})
        return {
            "content": [
                {
                    "type": "text",
