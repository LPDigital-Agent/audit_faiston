...
class InsightCategory(str, Enum):
    """Category of insight from ObservationAgent."""
    ERROR_PATTERN = "ERROR_PATTERN"              # Recurrent transformation errors
    MAPPING_OPPORTUNITY = "MAPPING_OPPORTUNITY"  # Unmapped columns with suggestions
    BEHAVIOR_INSIGHT = "BEHAVIOR_INSIGHT"        # User preference patterns
    HEALTH_ALERT = "HEALTH_ALERT"                # Database/inventory anomalies
    SYSTEM_HEALTH = "SYSTEM_HEALTH"              # Agent errors (fallback)


class InsightSeverity(str, Enum):
    """Severity level of an insight (Traffic Light system)."""
    CRITICAL = "critical"  # Data integrity/financial risk → Immediate action
    WARNING = "warning"    # Process friction/recurring errors → Add to backlog
    INFO = "info"          # Optimization/trends → FYI only


class InsightStatus(str, Enum):
    """Lifecycle status of an insight."""
    PENDING = "pending"      # Awaiting delivery to user
    DELIVERED = "delivered"  # Shown to user, awaiting action
    ACTIONED = "actioned"    # User took action (one-click fix)
    DISMISSED = "dismissed"  # User dismissed (with learning)
...
# =============================================================================
# Phase 5: ObservationAgent Sub-Models
# =============================================================================


class ActionPayload(BaseModel):
    """
    One-click execution payload for actionable insights.

    When an insight has a suggested fix, this payload contains the
    exact tool call needed to execute it. The orchestrator can invoke
    this directly without additional LLM reasoning.

    Example:
        ActionPayload(
            tool="system_update_mapping",
            params={"source": "SKU", "target": "part_number"}
        )
    """
    tool: str = Field(
        description="Tool name to invoke (e.g., 'system_update_mapping')"
    )
    params: Dict[str, Any] = Field(
        default_factory=dict,
        description="Tool parameters for execution"
    )


class InsightReport(BaseModel):
    """
    Structured insight from ObservationAgent.

    Represents a proactive recommendation or alert generated by analyzing
    historical data from AgentCore Memory and the inventory database.

    The insight follows the Traffic Light severity system:
    - CRITICAL: Data integrity/financial risk → Requires immediate action
    - WARNING: Process friction/recurring errors → Add to backlog
    - INFO: Optimization/trends → FYI only

    Insights support one-click fixes via action_payload when applicable.
    """
    insight_id: str = Field(
        description="Unique insight identifier"
    )
    category: InsightCategory = Field(
        description="Type of insight (error pattern, mapping, health, etc.)"
    )
    severity: InsightSeverity = Field(
        description="Traffic light severity (critical, warning, info)"
    )
    title: str = Field(
        max_length=100,
        description="Short title for display (e.g., 'Duplicate Part Numbers Detected')"
    )
    ...
    educational_context: str = Field(
        default="",
        description="LLM-generated explanation to help user understand (pt-BR)"
    )
    confidence: float = Field(
        ge=0.0, le=1.0,
    ...
    evidence: List[str] = Field(
        default_factory=list,
        description="Supporting evidence (e.g., 'Found 15 occurrences in last 7 days')"
    )
    suggested_action: Optional[str] = Field(
        default=None,
        description="Human-readable suggested action"
    )
    action_payload: Optional[ActionPayload] = Field(
        default=None,
        description="One-click fix payload (if applicable)"
    )
    human_message: str = Field(
        description="User-facing message in pt-BR"
    )
    created_at: str = Field(
        default="",
        description="ISO-8601 timestamp when insight was generated"
    )
    actor_id: Optional[str] = Field(
        default=None,
        description="User/actor this insight belongs to (for scoping)"
    )

    # Deduplication & lifecycle
    fingerprint: str = Field(
        default="",
        description="Hash for deduplication (category:target_entity)"
    )
    ...
    dismissal_count: int = Field(
        default=0,
        description="Number of times user dismissed this insight type"
    )
    last_dismissed_at: Optional[str] = Field(
        default=None,
        description="ISO-8601 timestamp of last dismissal"
    )
    severity_at_dismissal: Optional[InsightSeverity] = Field(
        default=None,
        description="Severity when last dismissed (for resurface logic)"
    )
...
class ObservationResponse(AgentResponseBase):
    """
    Response from ObservationAgent (Phase 5).

    Contains proactive insights from Memory analysis and database health checks.
    Supports dynamic batch sizing based on severity.
    """
    # Insight delivery
    insights: List[InsightReport] = Field(
        default_factory=list,
        description="Proactive insights generated by ObservationAgent"
    )
    total_pending: int = Field(
        default=0,
        description="Total pending insights (not just displayed)"
    )
    displayed_count: int = Field(
        default=0,
        description="Number of insights displayed (1 if critical, up to 3 otherwise)"
    )

    # Analysis context
    analysis_window_hours: int = Field(
        default=24,
        description="Time window analyzed (24h tactical, 168h weekly, 720h monthly)"
    )
    patterns_detected: int = Field(
        default=0,
        description="Number of patterns found during analysis"
    )

    # Health metrics (when health check is performed)
    health_score: Optional[float] = Field(
        default=None, ge=0.0, le=1.0,
        description="Overall inventory health score (0.0 - 1.0)"
    )
    anomaly_counts: Optional[Dict[str, int]] = Field(
        default=None,
        description="Count of anomalies by type (zero_stock, duplicates, etc.)"
    )

    # User-facing message
    human_message: str = Field(
        default="",
        description="Summary message in pt-BR for user"
    )
